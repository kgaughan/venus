...
def invalidate(c):
    ...
    return '<abbr title="U+%s">\ufffd</abbr>' % ("000" + hex(ord(c.group(0)))[2:])[-4:]
        ...
        for body in html.documentElement.childNodes:
            ...
            for div in body.childNodes:
                ...
                try:
                    div.normalize()
                    if (
                        len(div.childNodes) == 1
                        and div.firstChild.nodeType == Node.TEXT_NODE
                    ):
                        data = div.firstChild
...
def reconstitute(feed, entry):
    ...
    if "dc_date.taken" in entry:
        date_Taken = createTextElement(
            xentry,
            "%s:%s" % ("dc", "date_Taken"),
            "%s" % entry.get("dc_date.taken", None),
        )
    ...
    for ns, name in [("feedburner", "origLink")]:
        if "%s_%s" % (ns, name.lower()) in entry and ns in feed.namespaces:
            xoriglink = createTextElement(
                xentry, "%s:%s" % (ns, name), entry["%s_%s" % (ns, name.lower())]
            )
    ...
    if (
        "where" in entry
        and "type" in entry.get("where", [])
        and "coordinates" in entry.get("where", [])
    ):
    ...
    if author_detail and "name" not in author_detail and "planet_name" in feed.feed:
    ...
    if (
        (not author_detail or "name" not in author_detail)
        and "name" not in src_author
        and "planet_name" in feed.feed
    ):
        if src_author:
