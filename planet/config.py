...
def __init__():
    ...
    def define_tmpl(name, default):
        setattr(
            config, name, lambda section, default=default: get(section, name, default)
        )
    ...
    def define_tmpl_int(name, default):
        setattr(
            config,
            name,
            lambda section, default=default: int(get(section, name, default)),
        )
        ...
        def data2config(data, cached_config):
            if content_type(list).find("opml") >= 0:
                ...
                cached_config.readfp(
                    io.StringIO(
                        shell.run(content_type(list), data.getvalue(), mode="filter")
                    )
                )
...
def downloadReadingList(list, orig_config, callback, use_cache=True, re_read=True):
    ...
    try:
        ...
        if "last-modified" in response.headers:
            cached_config.set(list, "last-modified", response.headers["last-modified"])
...
def http_cache_directory():
    if parser.has_option("Planet", "http_cache_directory"):
        return os.path.join(
            cache_directory(), parser.get("Planet", "http_cache_directory")
        )
    else:
...
def cache_sources_directory():
    if parser.has_option("Planet", "cache_sources_directory"):
        return os.path.join(
            cache_directory(), parser.get("Planet", "cache_sources_directory")
        )
    else:
...
def cache_blacklist_directory():
    if parser.has_option("Planet", "cache_blacklist_directory"):
        return os.path.join(
            cache_directory(), parser.get("Planet", "cache_blacklist_directory")
        )
    else:
...
def subscriptions():
    ...
    return __builtins__["filter"](
        ...
        and feed not in template_files() + filters() + reading_lists(),
        parser.sections(),
    )
...
def filters(section=None):
    ...
    if exclude(section):
        filters.append(
            "regexp_sifter.py?exclude=" + urllib.parse.quote(exclude(section))
        )
...
def planet_options():
    ...
    return dict(
        [
            (opt, parser.get("Planet", opt, raw=(opt == "log_format")))
            for opt in parser.options("Planet")
        ]
    )
...
def feed_options(section):
    ...
    if parser.has_section(section):
        options.update(
            dict([(opt, parser.get(section, opt)) for opt in parser.options(section)])
        )
    return options
