...
type_map = {"text": "text/plain", "html": "text/html", "xhtml": "application/xhtml+xml"}
...
def scrub(feed_uri, data):
    ...
    if config.name_type(feed_uri).find("html") >= 0:
        ...
        if "author_detail" in data.feed and "name" in data.feed.author_detail:
            data.feed.author_detail["name"] = str(
                stripHtml(data.feed.author_detail.name)
            )
        for entry in data.entries:
            if "author_detail" in entry and "name" in entry.author_detail:
                entry.author_detail["name"] = str(stripHtml(entry.author_detail.name))
            if "source" in entry:
                source = entry.source
                if "author_detail" in source and "name" in source.author_detail:
                    source.author_detail["name"] = str(
                        stripHtml(source.author_detail.name)
                    )
    ...
    if future_dates == "ignore_date":
        ...
        data.entries = [
            ...
            if (
                ...
                or entry["published_parsed"] <= now
            )
            and (
                "updated_parsed" not in entry
                or not entry["updated_parsed"]
            ...
            if "base" in node:
                if scrub_xmlbase:
                    if scrub_xmlbase == "feed_alternate":
                        if "source" in entry and "link" in entry.source:
