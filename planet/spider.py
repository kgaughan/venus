    ...
    if len(filename) > 250:
        ...
        for i in range(len(parts), 0, -1):
            if len(",".join(parts[:i])) < 220:
                filename = (
                    ",".join(parts[:i]) + "," + md5(",".join(parts[i:])).hexdigest()
                )
                break
...
def writeCache(feed_uri, feed_info, data):
    ...
    activity_horizon = time.gmtime(
        time.time() - 86400 * config.activity_threshold(feed_uri)
    )
    ...
    if data.status == 200 and "url" in data:
        ...
        if "planet_message" not in feed_info.feed:
            ...
            if feed_info.feed.planet_message.startswith("no activity in"):
                return
            if not feed_info.feed.planet_message.startswith(
                "duplicate"
            ) and not feed_info.feed.planet_message.startswith("no data"):
    ...
    while uri:
        ...
        try:
            ...
            if "planet_http_last_modified" in feed_info.feed:
                headers["If-Modified-Since"] = feed_info.feed[
                    "planet_http_last_modified"
                ]
            ...
            if resp.status == 200:
                if resp.fromcache:
                    ...
                    "planet_content_hash" in feed_info.feed
...
def spiderPlanet(only_if_new=False):
    ...
    if int(config.spider_threads()):
        ...
        for i in range(int(config.spider_threads())):
            threads[i] = Thread(
                target=httpThread, args=(i, fetch_queue, parse_queue, log)
            )
            threads[i].start()
    else:
            ...
            try:
                ...
                if not hasattr(feed, "headers") or int(feed.headers.status) < 300:
                    ...
                    if hasattr(feed_info, "feed"):
                        options["etag"] = feed_info.feed.get("planet_http_etag", None)
                        try:
                ...
                if duplicate:
                    ...
                    log.warn(
                        "Duplicate subscription: %s and %s"
