    ...
    for statement in model.find_statements(Statement(subject, foaf.weblog, None)):
        ...
        if accounts:
            for statement in model.find_statements(
                ...
                if (
                    not rdfaccthome.is_resource()
                    or str(rdfaccthome.uri) not in accounts
                ):
                    continue
            ...
            for statement in model.find_statements(Statement(person, foaf.knows, None)):
                ...
                if not config.has_section(seeAlso):
                    ...
                    copy_options(
                        ...
                        section,
                        seeAlso,
                        {"content_type": "foaf", "depth": str(depth - 1)},
                    )
                try:
                    ...
                    downloadReadingList(
                        seeAlso,
                        config,
                        lambda data, subconfig: friend2config(
                            model, friend, seeAlso, subconfig, data
                        ),
                        False,
                    )
                except:
...
def copy_options(config, parent_section, child_section, overrides={}):
    ...
    for option in [
        x for x in config.options(parent_section) if x in inheritable_options
    ]:
