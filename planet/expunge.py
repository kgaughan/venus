    ...
    for file in dir:
        try:
            ...
            if ids[0].childNodes[0].nodeValue in entry_count:
                ...
                entry_count[ids[0].childNodes[0].nodeValue] = (
                    entry_count[ids[0].childNodes[0].nodeValue] - 1
                )
                if entry_count[ids[0].childNodes[0].nodeValue] >= 0:
                    # maximum not reached, do not delete
                    log.debug(
                        "Maximum not reached for %s from %s",
                        file,
                    ...
                    # maximum reached
                    log.debug(
                        "Removing %s, maximum reached for %s",
                        file,
                        ids[0].childNodes[0].nodeValue,
                    )
            else:
                # not subscribed
                log.debug(
                    "Removing %s, not subscribed to %s",
                    file,
                    ids[0].childNodes[0].nodeValue,
                )
            # remove old entry
