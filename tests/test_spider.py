...
class SpiderTest(unittest.TestCase):
    ...
    def test_filename(self):
        ...
        self.assertEqual(
            os.path.join(".", "planet.intertwingly.net,2006,testfeed1,1"),
            filename(".", "tag:planet.intertwingly.net,2006:testfeed1,1"),
        )
        self.assertEqual(
            os.path.join(".", "00000000-0000-0000-0000-000000000000"),
        ...
        try:
            ...
            return
        self.assertEqual(
            os.path.join(".", "xn--8ws00zhy3a.com"),
    ...
    def verify_spiderFeed(self):
        ...
        self.assertTrue(
            os.path.join(workdir, "planet.intertwingly.net,2006,testfeed1,1") in files
        )
        ...
        self.assertEqual(
            ...
            [link.type for link in data.entries[0].source.links if link.rel == "self"],
        )
    ...
    def test_spiderFeed_blacklist(self):
        ...
        os.rename(
            os.path.join(workdir, "planet.intertwingly.net,2006,testfeed1,1"),
            os.path.join(
                workdir, "blacklist", "planet.intertwingly.net,2006,testfeed1,1"
            ),
        )
    ...
    def test_spiderFeedUpdatedEntries(self):
        ...
        data = feedparser.parse(workdir + "/planet.intertwingly.net,2006,testfeed4")
    ...
    def verify_spiderPlanet(self):
        ...
        self.assertTrue(
            os.path.join(workdir, "planet.intertwingly.net,2006,testfeed2,1") in files
        )

        data = feedparser.parse(workdir + "/planet.intertwingly.net,2006,testfeed3,1")
        self.assertEqual(
            ["application/rss+xml"],
            [link.type for link in data.entries[0].source.links if link.rel == "self"],
        )
